...
void writeToFileName(int n, dataName source[], char fileName[]){
    ...
    for(int i = 0; i < n; i++){
        fprintf(fTemp, "%s %s %d\n", source[i].judul, source[i].musisi, source[i].tahun); // write data ke dalam file
    }
...
void readFileName(int *n, dataName source[], char fileName[]){
    ...
    if(strcmp(source[*n].judul, "####") == 0 || returnVal == EOF) printf("File Kosong\n");
    else{ // jika bukan EOF
        while(strcmp(source[*n].judul, "####") != 0){ // selama bukan data EOF
            *n = *n + 1; // maju ke indeks penampung berikutnya
            fscanf(fTemp, "%s %s %d", source[*n].judul, source[*n].musisi, &source[*n].tahun); // scan data dan menampungnya dalam array sesuai index ke-*n, kemudian scan data berikutnya
        }
    }
